...
Supabase backs the *add product* and *search products* pages, and holds the schema that
...
This project uses Supabase's newer key format. The dashboard also has a *Legacy anon,
service_role API keys* tab — ignore it. The variable names still say `ANON` and
...
Adding auth will need a *second* client (browser + SSR, publishable key, session-aware).
...
3. SQL Editor → run `0001_init.sql`, then `0002_auth.sql`.
   `0002` triggers a *"destructive operations"* warning because of its `drop policy` /
